...
class FilmConvBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, factor=1, bidirectional_mamba=False):
        super().__init__()
...
class BFilmConvBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel):
        super().__init__()
        # self. proj not really projecting anything! todo: mamba
        self.proj = Conv1d(in_channel, out_channel, 3, padding=1)
        self.act = nn.SiLU()
        self.conv = Conv1d(out_channel, out_channel, 3, padding=1)
...
class GBlock(nn.Module):
    def __init__(self, in_channel, out_channel, factor, block_num, film_type, event_dim, bidirectional_mamba:bool=False):
        super().__init__()
        self.factor = factor
        if self.factor < 0: in_channel = in_channel * 2
        mamba_channels = int(in_channel / 2)

        self.conv_pre_mamba = nn.Conv1d(in_channels=in_channel, out_channels=mamba_channels, kernel_size=3, padding=1)
        self.mamba = MambaBlock(
                    in_channels=mamba_channels,
                    n_layer=1,
                    bidirectional=bidirectional_mamba
                )
        self.conv_post_mamba = nn.Conv1d(in_channels=mamba_channels*(bidirectional_mamba+1), out_channels=in_channel, kernel_size=3, padding=1)
